...
There are two options that can be used to plot the NPS Value:
...
# Option One: Visualise using Bar Plot {#BarPlot}
...
In order to properly visualise the NPS score, an empty data frame is generated, with one row being eich of the possible scores. The reason for this will be explained in a later step. The way that this data is generated is by using the `seq()` function to create an ordered sequence of numbers from $0$ to $10$, incrementing by $1$ each time.
...
Finally, the result is plotted using the `ggplot` package and the following aesthetics: `geom_bar()`, `geom_point()`, and `geom_label()`.
...
1. Add a `geom_point()` layer, using the following arguments:
    ...
    1. '`data`' is created using an anonymous function. This is so that the data used by the `ggplot()` function can be manipulated, without using another external variable. The manipulation was effecively used to create a single NPS score which can be used in this layer.
...
1. Determine the colours that should be used in the three differenet Categories, using the `scale_fill_manual()` layer.
...
# Option Two: Visualise using density plot {#DensityPlot}
...
This is where you write a conclusion.
...
1. GitHub: [GitHub/chrimaho/AusEnergyPrices](https://github.com/chrimaho/AusEnergyPrices)
...
1. LinkedIn: [LinkedIn/chrimaho/AusEnergyPrices](https://www.linkedin.com/pulse/future-australian-energy-prices-chris-mahoney)
